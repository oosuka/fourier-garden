class FourierGardenProcessor extends AudioWorkletProcessor {
  ...
  process(_inputs, outputs) {
    ...
    for (let frame = 0; frame < left.length; frame += 1) {
      ...
      const attackProgress = Math.min(1, Math.max(0, localTime / rhythm.attackSeconds));
      ...
      const releaseProgress = Math.min(
        1,
        Math.max(0, (rhythm.stepSeconds - localTime) / rhythm.releaseSeconds),
      );
      ...
      for (let index = 0; index < this.partials.length; index += 1) {
        ...
        if (frequency >= sampleRate * 0.5 * rhythm.antiAliasRatio) {
          continue;
        }
        const gain = partial.sourceAmplitude / Math.pow(index + 1, rhythm.timbreDamping);
        ...
        leftSample += Math.sin(leftPhase) * gain * Math.sqrt((1 - pan) * 0.5);
        rightSample += Math.sin(rightPhase) * gain * Math.sqrt((1 + pan) * 0.5);
        normalization += gain;
      }

      const scale = normalization > 0 ? rhythm.outputGain / normalization : 0;
      left[frame] = leftSample * scale * envelope * this.fade;
      right[frame] = rightSample * scale * envelope * this.fade;
