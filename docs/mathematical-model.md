...
\[
H=[1.30,0.28,0.72,0.38,1.04,0.24,0.62,0.34,1.18,0.30,0.68,0.40,0.94,0.22,0.56,0.36]
\]

を使い、発音slot \(s\)では\(H_{(s+3b_s)\bmod16}\)を乗じる。
評価済みアクセントは\([0.18,1.46]\)へ制限する。さらに小節内の時計とは別に、
8小節の基礎ゲイン弧

\[
G=[0.72,0.90,1.08,0.82,0.96,1.16,0.78,1.04]
\]

をsection内で巡回する。これは全16分slotを鳴らしたまま、
...
440 Hzキャリアで13項、495 Hzキャリアで11項が採用される。帯域条件で一部を
除いた場合も、等電力定位係数\(P_k^{L/R}\)は圧縮後の配列位置ではなく元の調波index
\(k\)から評価する。したがって標本レートによる上側調波の省略が、残った調波の
定位規約を並べ替えることはない。
...
Worklet stateは設定時にフェーザ評価領域、評価済みイベント、出力標本を一度だけ確保する。
標本ループでは周回番号とglobal stepを数値で比較してイベント評価を更新し、文字列キー、
一時配列、一時オブジェクトを生成せず、同じ領域へ結果を書き戻す。
...
このうち局所粒子はWebGPUで`4,000 / 7,000 / 9,000 / 12,000`、発音バーストは
4 slot合計768点である。残りを全画面環境粒子へ割り当てる。WebGL2では局所粒子を
`3,000 / 5,000 / 7,000 / 9,000`へ抑え、全画面環境粒子の実描画上限を8,000点にする。
環境層は3深度帯の粒子、5枚の星雲ベール、6本のフィラメントベール、
7層の共鳴ハロー、9個の局所フレアを持つ。共鳴ハローとフレアは詩的造形であり、
厳密なフェーザ半径や解析係数を表さない。
...
利用者による先行版の音色評価は2026年7月11日に完了し、2026年7月12日の正式版として
承認された。2026年8月12日の最後の全章比較でもChapter 2は心地よい基準方向として評価され、
翌日の全章再調整では乾いた中央寄りのキャラクターを維持している。
...
5枚の星雲ベール、6本のフィラメントベール、7層の共鳴ハロー、9個の局所フレアを持つ。
WebGL2では環境粒子の実描画上限を
8,000点にし、体積ハロー数をhigh以下で先に削減する。

WebGPUとWebGL2は品質別のbloom profileを使い、highではstrength 1.2、
radius 0.38、threshold 0.76を基準にする。高いthresholdで深い黒を保持しながら、
柱芯、透明ハロー平面、局所フレア、
...
WebGL2の実ラスタが600万pixelを超える場合は、UnrealBloomの全画面追加passだけを
停止し、rendererをネイティブ解像度の直接描画へ切り替える。これは詩的造形層の
性能制御であり、数学面、境界、節線、係数、数学時刻、局所粒子配置を変更しない。
...
| gesture | attack | 主減衰 | fade開始 | 終了 | subgrain offset / gain |
| --- | ---: | ---: | ---: | ---: | --- |
| toll | 8 ms | 48 ms | 122 ms | 150 ms | 0 ms / 0.86 |
| answer | 8 ms | 50 ms | 122 ms | 150 ms | 0, 86 ms / 0.92, 0.18 |
| cascade | 6 ms | 46 ms | 116 ms | 145 ms | 0, 43, 86 ms / 1.00, 0.16, 0.18 |
| pulse | 6 ms | 44 ms | 112 ms | 140 ms | 0, 86 ms / 1.00, 0.18 |
| choir | 9 ms | 52 ms | 126 ms | 155 ms | 0, 86 ms / 0.80, 0.18 |
...
ピコ粒へ写す。隣接素数間隔を\(\Delta_j\)とすると発音間隔は
\((9.2/95)\Delta_j\)秒で、25素数を9.2秒へ比率保存で配置する。
次の10秒句までの間隔は0.8秒であり、数学順序を変えずに句を連続させる。
DFT、FFT、確率的素数推定は使用しない。
...
\[
f_{mn}=420+
\left(\frac{\sqrt{\lambda_{mn}}-\sqrt{\lambda_{\min}}}
{\sqrt{\lambda_{\max}}-\sqrt{\lambda_{\min}}}\right)^{1.65}(920-420)\ \mathrm{Hz},
\qquad
...
包絡終了は0.22-0.23秒、評価窓の最大イベント長は0.23秒で、16分格子上の重なりと
...
標本ループでは時間窓内のeventだけを評価する。carrier位相と
\(\phi_i(t)\)の正弦・余弦は発音開始時またはseek時に絶対時刻から初期化し、
以後は標本間位相差の複素回転で更新する。公開DSPではbreath gainが0なので、
breathとairの無音付加経路は標本ループで評価しない。これは上式の絶対時刻、
振幅、定位、quadrature関係を変えず、無効な付加音源の計算だけを除く。
Web Audioグラフはdry high-pass 220 Hz、high-shelf 900 Hz / -28 dB、
dry low-pass 960 Hz、wet high-pass 220 Hz、wet low-pass 720 Hz、
1.2秒残響、音響正規化後の出力ゲイン0.303039688456、threshold -16 dBの圧縮、
...
外側膜と霧は詩的造形である。零点順を420-940 Hzへ正規化し、係数比は
`mathematicalGain`へ厳密に保持する。発音強度だけは係数絶対値の平方根による
単調な知覚圧縮を施し、弱い固有モードも消さずに6/8の滴状ピコへ写す。
Dirichlet零点とNeumann零点を混同しない。
...
比\(a:b\)は句順と位相関係へ保持し、carrierは中域へ置く。
曲線パラメータ\(s\)と絶対transport時刻\(t\)を分離する。60秒周期を9個の
32イベント句へ分け、各句では画面の主曲線と同じFarey比を保持する。各イベントの
\(s\)で得る厳密座標\((x,y)=\Gamma_{a,b}(s,t)\)を中域の音高輪郭
\(440+12i+90(x+1)+50(y+1)\) Hzと定位へ写すため、句内でも同じ音高を固定反復しない。
pan motion rate、phase driftもその比から導き、短いイベントごとに別の比へ飛ばさない。
...
符号を包絡、定位、強度、開始位相へ写す。各発音時刻ではscoreの`sourceIndex`から
同じHaar係数を選び、その半開支持の中心へ走査線を置く。厳密な係数セルの局所応答と
区分一定再構成を維持したまま、同じ支持から落下する係数滴と衝突環だけを詩的造形層として
加える。音楽時刻は平均0.2秒を保ちながら、16 slotの間隔を
`0.10, 0.10, 0.28, 0.32, 0.10, 0.10, 0.12, 0.48`秒の二進クラスタ2組へ分ける。
carrierは420-730 Hz、有限包絡は平均0.28秒以上とし、等間隔で乾いたDirichlet Lanterns
から、暗く重なる下降雨として分離する。
...
無限和の微分可能性、自己相似性、ゼータ零点を主張しない。平方数順序、主声部の
1/n²比、絶対位相を中域のガラス質ピコへ写し、句の連続性を保つ応答声部だけは
知覚圧縮した強度を使う。各16秒幕の主発音は
\(t_n=16(n-1)^2/19^2\)を保つ。次の主時刻を\(t_{20}=16\)と補い、応答声部は
\[
t_{n,q}=t_n+\frac q3(t_{n+1}-t_n),\qquad q\in\{1,2\}
\]
へ置く。二次間隔の拡大を変えずに各区間を3等分し、全発音列の最大間隔を
0.55秒以下へ分割する。
応答声部は主声部より長めの基礎有限包絡で隣接時刻へ柔らかく重なり、主声部の
\(1/n^2\)比を変えずに長い無音と冒頭の過大な瞬間ピークを抑える。主声部carrierは

\[
\nu_n=460+300\left(\frac{n-1}{18}\right)^{0.72}\ \mathrm{Hz}
\]

とし、2つの応答はそれぞれ\(0.875\nu_n\)、\(0.75\nu_n\)へ下行させ、下限を380 Hzとする。
最大carrierは760 Hz、部分音gainは0.025、dry high-shelfは900 Hz / -28 dB、
dry low-passは1,050 Hzとする。平方数順序と主声部の係数比を保ったまま、Mac内蔵
スピーカーで刺激になった上側中域をソニフィケーション層だけで抑える。
...
7/8の位相変調ピコと回転定位へ写す。420イベントの平均0.2秒時計は、20 slot・4秒の
軌道句内で0.08-0.36秒の非一様間隔へ再配置する。`mathematicalGain`には係数比を厳密に
保持し、可聴gainだけへ平方根圧縮を施して弱い文字を残す。平均0.33秒以上の有限尾と
1.18秒残響により、等間隔の点列ではなく連続する無理比軌道として聴かせる。
...
### Chapter 3と5から10の章固有音色

Chapter 3と5から10は共通の有限ピコ包絡を維持しながら、章固有の低ゲイン部分音と
微小chirpで音色を分離する。イベントageを\(\tau\)、有限終了時刻を\(T_e\)、
基音周波数を\(\nu\)、開始位相と絶対時刻位相をまとめた値を\(\phi(t)\)とし、

\[
\chi_c(\tau)=\tau+\kappa_c\left(\tau-\frac{\tau^2}{2T_e}\right),
\]

\[
q_c(\tau,t)=
\frac{
\sin\left(2\pi\nu\chi_c(\tau)+\phi(t)\right)
+\beta_c\sin\left(2\pi\rho_c\nu\chi_c(\tau)+\phi(t)\right)
}{\sqrt{1+\beta_c^2}}
\]
...
\[
\max(\nu_L,\nu_R)\rho_c\left(1+\max(0,\kappa_c)\right)
\]

が\(0.45F_s\)以上になる標本レートでは、そのイベントの\(\beta_c\)だけを0とし、
安全な基音は失わない。これは数学係数の変更ではなく、ソニフィケーション層の
帯域制限付き音色設計である。

| Chapter | \(\rho_c\) | \(\beta_c\) | \(\kappa_c\) | 聴感上の役割 |
| --- | ---: | ---: | ---: | --- |
| 3 Prime Constellation | 3/2 | 0.055 | 0.012 | 硬い素数粒と上向きの緊張 |
| 5 Bessel Tide | 2 | 0.100 | -0.032 | 丸い倍音と下降する水滴 |
| 6 Lissajous Orchard | 4/3 | 0.045 | -0.014 | 曲線座標をたどる暖かい弦状輪郭 |
| 7 Dirichlet Lanterns | 3 | 0.045 | 0 | 乾いた奇数支持の灯 |
| 8 Wavelet Rain | 5/2 | 0.030 | -0.038 | 暗い二進クラスタの落下感 |
| 9 Riemann Veil | \(\sqrt2\) | 0.025 | -0.012 | 下行応答を持つ丸い非整数比 |
| 10 Phase Torus | \((1+\sqrt5)/2\) | 0.055 | 0.022 | 無理比の持続する回転感 |

AudioWorkletでは上式の位相を変えず、イベント境界で初期位相、標本間位相差
\(\Delta_n\)、chirpによる位相差の差分\(\delta\)を一度だけ求める。標本ループでは

\[
\begin{aligned}
s_{n+1}&=s_n\cos\Delta_n+c_n\sin\Delta_n,\\
c_{n+1}&=c_n\cos\Delta_n-s_n\sin\Delta_n,\\
\sin\Delta_{n+1}
&=\sin\Delta_n\cos\delta+\cos\Delta_n\sin\delta,\\
\cos\Delta_{n+1}
&=\cos\Delta_n\cos\delta-\sin\Delta_n\sin\delta
\end{aligned}
\]

の複素回転漸化式で基音と部分音を更新する。Chapter 1の13調波もchirpを持たない
同じ回転形式で更新する。これは標本ごとの三角関数呼び出しを除く実装最適化であり、
参照式、開始位相、絶対transport時刻、帯域条件を変更しない。
共有ピコの有限包絡は2つの指数減衰成分、絶対時刻pan motionは正弦・余弦の
標本間回転として同じ発音境界で初期化し、標本ループでは乗算と加算だけで更新する。
...
| --- | ---: |
| 1 Residue Bloom | 0.186903991 |
| 2 Spectral Cathedral | 0.506816908 |
| 3 Prime Constellation | 0.408405599 |
| 4 Möbius Choir | 0.303039688 |
| 5 Bessel Tide | 1.021004366 |
| 6 Lissajous Orchard | 0.340966473 |
| 7 Dirichlet Lanterns | 0.665008814 |
| 8 Wavelet Rain | 0.443262032 |
| 9 Riemann Veil | 1.149409291 |
| 10 Phase Torus | 0.408303271 |
...
共有ピコの定位は、章固有の数学写像と絶対時刻pan motionを評価した後、全周期の
乾音エネルギー重心だけを左右中央へ平行移動する。局所的な左右順序、距離差、運動速度は
変えず、有限スコアの片側偏りによる長時間聴取疲労だけを除くソニフィケーション層の
マスタリング処理である。
