...
describe("Residue Bloom Fourier series", () => {
  ...
  it("uses the phasor imaginary projection as the direct series value", () => {
    for (let sample = 0; sample <= 256; sample += 1) {
      ...
      const rawEndpoint = getEpicycleSteps(RESIDUE_BLOOM_SERIES, angle).at(-1);
  ...
  it("reports the conventional two-sided complex Fourier coefficients", () => {
    const coefficients = getComplexFourierCoefficients(RESIDUE_BLOOM_SERIES);
    const positive = coefficients.find((coefficient) => coefficient.harmonic === 1);
    const negative = coefficients.find((coefficient) => coefficient.harmonic === -1);
  ...
  it("projects the primary waveform with the same center and scale as the phasor", () => {
    ...
    expect(projectSeriesToVerticalAxis(RESIDUE_BLOOM_SERIES, angle, centerY, scale)).toBeCloseTo(
      centerY + endpoint.y * scale,
      12,
    );
  });
