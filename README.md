...
主波形と同じ射影点へ短い別ラインと光点を重ねます。変化するのは色、opacity、
光点サイズだけです。
...
イベント表はアプリ起動時に一度だけ生成され、区間、発音、carrier、
基礎プロファイルだけを保持します。フェーザ座標、半径、フェーザ由来の
brightness・accentは保存しません。AudioWorkletは渡されたイベント表と
フェーザ写像をサンプル精度で評価し、区間、発音マスク、carrier列を
独自に再定義しません。

各発音の絶対イベント時刻`tₑ`では`z(0.31tₑ)`を`ΣAₖ`で正規化し、実部を定位、
虚部を後段low-passの明るさ、絶対値をアクセントと減衰へ有界に写像します。
残響送出は区間プロファイルから決まります。粒子のburstはその履歴終点を
...
ハロー、burst、膜、flow、bloom、調波コロナ、履歴パルスを決定的に再構成します。
